...
When a generated round qualifies as a BIG ODD, the bootstrap
publishes it here immediately.
...
The REST API only serves records received from the game server.
=========================================================
*/
...
function normalizeStatus(status) {
    if (status === "played" || status === "CRASHED") {
        return "played";
    }
...
function publishFromRound(round) {
    ...
    const record = {
        ...
        runningAt:
            status === "running"
                ? (round.startedAt || round.serverTime || null)
                : null,
        playedAt:
            status === "played"
                ? (round.crashedAt || round.serverTime || null)
                : null,
